...
// project_name 正規化,對齊 wez-html:trim、必須單行、最多 80 個字元(rune)。
// 超過或含換行 → throw Error(呼叫端接住回 400)。
export const PROJECT_NAME_MAX_RUNES = 80;
export function normalizeProjectName(raw) {
  const projectName = (raw || "").trim();
  if (/[\r\n]/.test(projectName)) {
    throw new Error("project_name must be a single line");
  }
  // 用 spread 數 code point(rune),避免 surrogate pair 被算兩個
  if ([...projectName].length > PROJECT_NAME_MAX_RUNES) {
    throw new Error(
      `project_name must be at most ${PROJECT_NAME_MAX_RUNES} characters`
    );
  }
  return projectName;
}
